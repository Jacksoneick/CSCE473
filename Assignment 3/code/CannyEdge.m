...
T_Low = LowT * max(max(output));
T_High = HighT * max(max(output));

T_res = zeros (h, w); %Our Threshold Matrix starts out like this

for i = 1  : h
    for j = 1 : w
        if (output(i, j) < T_Low)
            T_res(i, j) = 0;
        elseif (output(i, j) > T_High)
            T_res(i, j) = 1;
        %Looks for points connecting to an edge to continue along the edge
        elseif ( output(i+1,j)>T_High || output(i-1,j)>T_High || output(i,j+1)>T_High || output(i,j-1)>T_High || output(i-1, j-1)>T_High || output(i-1, j+1)>T_High || output(i+1, j+1)>T_High || output(i+1, j-1)>T_High)
            T_res(i,j) = 1;
        end;
    end;
end;
